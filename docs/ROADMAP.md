...
### RocketRide wiring — LOCKED architecture, pipeline built, not yet deployed to Cloud
...
**Update 2026-07-07 (later):** the pipeline itself is now built — `constructor-pipeline.pipe` at repo root (webhook → question → `agent_rocketride` with LLM/memory/`tool_http_request` → response; LLM is MiniMax M3, not Anthropic). All required credentials (`ROCKETRIDE_URI`/`ROCKETRIDE_APIKEY` platform auth, `ROCKETRIDE_MINIMAX_KEY`, `ROCKETRIDE_DAYTONA_URL`, `ROCKETRIDE_TRIGGER_SCOUT_URL`) are filled in `.env` — the pipeline is runnable now. Full shape, contract, and remaining open items (response envelope unverified, `project_id` churn in the RocketRide extension sync) are in `docs/ROCKETRIDE_REQUIREMENTS.md` §8-9 — read that before touching this further. Also: `trigger-scout`'s deployed trigger was flipped from `auth:'required'` to `auth:'none'` (confirmed via Butterbase MCP that `'required'` flatly rejects service keys, wanting an end-user JWT this project has no way to produce) — code/Cypher/env vars unchanged, only the trigger auth mode. **Only remaining gap: not yet deployed to cloud.rocketride.ai** (no CLI/API for this — it's a manual "Deploy" click in the RocketRide VS Code extension); `ROCKETRIDE_PIPELINE_URL` still unset on `trigger-analyze` until that happens.
...
- **Ramis + Rohan (RocketRide)** — ⚠️ pipeline built (`constructor-pipeline.pipe`), **not yet deployed to RocketRide Cloud**, `trigger-analyze` not yet repointed. See `docs/ROCKETRIDE_REQUIREMENTS.md` §8-9 for shape + open items.
...
3. Is the RocketRide pipeline actually deployed to RocketRide Cloud (not just running locally)? — **BUILT, CREDENTIALS SET, NOT YET DEPLOYED.** `constructor-pipeline.pipe` exists, its component wiring is complete, and all required `.env` credentials are filled in (see `docs/ROCKETRIDE_REQUIREMENTS.md` §8-9) — but it hasn't been pushed live to cloud.rocketride.ai yet. That's a manual click ("Deploy" in the RocketRide VS Code extension) — no CLI/API path exists for it. This is an organizer must-have; don't skip it even under time pressure.
