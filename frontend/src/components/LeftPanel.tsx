...
function Spinner() {
  return (
    <span className="inline-block h-3 w-3 animate-spin rounded-full border-[1.5px] border-sky-500/30 border-t-sky-500" />
  );
}
    ...
    <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
      <div className="mb-1 flex items-center justify-between">
        ...
        {!editing && (
          <button
            onClick={startEdit}
            className="text-[10px] font-medium text-sky-600 hover:text-sky-500"
          >
            Edit
          </button>
        )}
      ...
      {editing ? (
        <div className="space-y-2">
          <textarea
            ...
            rows={4}
            className="w-full resize-none rounded-md border border-slate-300 bg-white p-2 text-xs leading-relaxed text-slate-800 outline-none focus:border-sky-500"
          />
          <div className="flex items-center justify-end gap-2">
            <button
              onClick={cancel}
              className="rounded px-2 py-1 text-[11px] font-medium text-slate-500 hover:text-slate-700"
            >
              Cancel
            </button>
            <button
              onClick={save}
              disabled={!draft.trim()}
              className="rounded bg-sky-600 px-2.5 py-1 text-[11px] font-medium text-white hover:bg-sky-500 disabled:opacity-40"
            >
              Save
            </button>
          </div>
        ...
        <p onClick={startEdit} className="cursor-text text-xs leading-relaxed text-slate-700">
          {goal}
        </p>
      )}
    </div>
    ...
    >
      ...
      <span
        className={`relative h-4 w-7 shrink-0 rounded-full transition-colors ${
          on && !exhausted ? 'bg-white/50' : 'bg-slate-300'
        }`}
      >
        <span
...
export function LeftPanel({
  ...
  return (
    <div className="flex h-full flex-col gap-4 p-4">
      <div>
        <h1 className="text-sm font-semibold tracking-wide text-sky-600">CONSTRUCTOR</h1>
        <p className="text-[11px] text-slate-400">Graph-native Research Command Center</p>
      </div>
      ...
      <div className="space-y-2">
        ...
        <button
          onClick={onAnalyze}
          disabled={busy}
          className="w-full rounded-md bg-orange-500 px-3 py-2 text-sm font-medium text-white hover:bg-orange-400 disabled:opacity-40"
        >
          Analyze
        </button>
        <button
          ...
          className="w-full cursor-not-allowed rounded-md bg-slate-100 px-3 py-2 text-sm font-medium text-slate-400"
        >
          Plan Next
        </button>
      </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        {running.length > 0 && (
          <div className="mb-2 rounded-lg border border-sky-200 bg-sky-50 px-2.5 py-2">
            <div className="mb-1.5 flex items-center gap-1.5 text-[10px] font-medium text-sky-700">
              <Spinner />
            ...
            <div className="space-y-1">
              {running.map((job) => (
                <div key={job.id} className="flex items-center gap-2 text-[11px]">
                  ...
                  <span className="flex-1 truncate text-slate-700">{jobLabel[job.type]}</span>
                  <span className="shrink-0 font-mono text-[9px] text-sky-500">
                    #{job.id.slice(-4)}
                  </span>
                </div>
              ))}
        ...
        <div className="flex-1 space-y-1.5 overflow-y-auto">
          {jobs.length === 0 && (
            <p className="text-xs text-slate-400">No jobs yet. Click Scout to start.</p>
          )}
          {jobs.map((job) => (
            <div
              key={job.id}
            ...
            >
              ...
              <span className="flex-1 truncate text-slate-600">{jobLabel[job.type]}</span>
